...
class TaskSerializer(serializers.ModelSerializer):
    ...
    def get_links(self, obj):
        request = self.context['request']
        links =  {
            'self':reverse('tasks-detail', kwargs={'pk':obj.pk},request=request),
            'sprint':None,
            'assigned':None
        }
        if obj.sprint_id:
        ...
        if obj.assigned:
            links['assigned'] = reverse('users-detail', kwargs={User.USERNAME_FIELD: obj.assigned}, request=request)
        return links
...
class UserSerializer(serializers.ModelSerializer):
    ...
    def get_links(self, obj):
        ...
        return {
            'self':reverse('users-detail', kwargs={User.USERNAME_FIELD: username}, request=request),
        }